...
Spójrz na fragment kodu poniżej. Klasa ta reprezentuje zakres liczb, ma ona jedną metodę, która sprawdza czy liczba przekazana jako argument należy do danego zakresu.
    ...
    public class Range { private final long lowerBound; private final long upperBound; public Range(long lowerBound, long upperBound) { this.lowerBound = lowerBound; this.upperBound = upperBound; } public boolean isInRange(long number) { return number >= lowerBound && number <= upperBound; }}

  
Poniżej przykład prostego testu jednostkowego, który sprawdza czy, liczba 15 jest w zakresie liczb od 10 do 20.

    @Testpublic void shouldSayThat15rIsInRange() { Range range = new Range(10, 20); Assert.assertTrue(range.isInRange(15));}
...
W pierwszej linijce importujemy metodę `assertFalse` z klasy `Assert`, druga linijka to importowanie wszystkich metod statycznych z tej klasy. Dzięki takim importom później w definicji klasy nie musimy używać nazwy klasy używając danej metody statycznej:
    ...
    assertFalse(false);assertTrue(true);
...
Czasami zdarza się, że chcemy przetestować pewną sytuację wyjątkową. Na przykład nie powinniśmy móc utworzyć instancji klasy `Range` z niepoprawnymi argumentami.
    ...
    public Range(long lowerBound, long upperBound) { if (lowerBound > upperBound) { throw new IllegalArgumentException("lowerBound is bigger than upperBound!"); } this.lowerBound = lowerBound; this.upperBound = upperBound;}
...
Z pomocą w takiej sytuacji przychodzi element `expected` adnotacji `@Test`. Przykład jego użycia widzisz poniżej:
    ...
    @Test(expected = IllegalArgumentException.class)public void shouldThrownIllegalArgumentExceptionOnWrongParameters() { new Range(20, 10);}
...
Istnieje też inny sposób. Możesz go użyć jeśli chcesz mieć dostęp do instancji rzuconego wyjątku. Pokazałem go w przykładzie poniżej:
    ...
    @Testpublic void shouldHaveProperErrorMessage() { try { new Range(20, 10); fail("Exception wasn't thrown!"); } catch (IllegalArgumentException exception) { assertEquals("lowerBound is bigger than upperBound!", exception.getMessage()); }}
...
Czasami zdarza się, że kilka testów jednostkowych wymaga pewnego „przygotowania”. Na przykład trzeba utworzyć instancję, którą będziemy później testowali. Twórcy biblioteki JUnit przyszli nam z pomocą. Istnieje adnotacja `@Before`, którą możemy dodać do metody w klasie z testami. Metoda ta zostanie uruchomiona przed każdym testem jednostkowym. Proszę spójrz na przykład poniżej.
    ...
    public class RangeTest { private Range range; @Before public void setUp() { range = new Range(10, 20); } @Test public void shouldSayThat15rIsInRange() { assertTrue(range.isInRange(15)); } @Test public void shouldSayThat5IsntInRange() { assertFalse(range.isInRange(5)); }}
...
[![nowy_test_dialog](http://www.samouczekprogramisty.pl/wp-content/uploads/2016/10/nowy_test_dialog-150x150.png)](http://www.samouczekprogramisty.pl/wp-content/uploads/2016/10/nowy_test_dialog.png)
...
[![nowy_test_dialog2](http://www.samouczekprogramisty.pl/wp-content/uploads/2016/10/nowy_test_dialog2-150x150.png)](http://www.samouczekprogramisty.pl/wp-content/uploads/2016/10/nowy_test_dialog2.png)
...
[![test_generowanie_kodu](http://www.samouczekprogramisty.pl/wp-content/uploads/2016/10/test_generowanie_kodu-150x150.png)](http://www.samouczekprogramisty.pl/wp-content/uploads/2016/10/test_generowanie_kodu.png)
...
[![testy_ok](http://www.samouczekprogramisty.pl/wp-content/uploads/2016/10/testy_ok-150x150.png)](http://www.samouczekprogramisty.pl/wp-content/uploads/2016/10/testy_ok.png)
