...
- [Wstęp do aplikacji webowych](http://www.samouczekprogramisty.pl/wprowadzenie-do-aplikacji-webowych/)
- [Wstęp do Gradle](http://www.samouczekprogramisty.pl/wstep-do-gradle/)
...
Adres URL może składać się z kilku części
    ...
    (scheme://)(user:password@)host(:port)(/)(path)(?query)(#fragment)http://marcin:tajnehaslo@www.samouczekprogramisty.pl:80/kurs-programowania-java?parametr=wartosc&innyParametr=wartosc#xxx
...
W artykule opisującym [Javę z linii poleceń](http://www.samouczekprogramisty.pl/java-z-linii-polecen/) możesz przeczytać o plikach jar. W przypadku aplikacji webowych plik war pełni kluczową rolę.
...
Podobnie jak w przypadku pliku jar jest to zwykłe archiwum zip ze zmienionym rozszerzeniem (war a nie zip). Buduje się go przy pomocy tych samych narzędzi jak plik jar . Struktura przykładowego pliku war jest następująca:
    ...
    struktura_pliku.war├── index.html├── publiczny_katalog│ └── strona.html├── style.css└── WEB-INF ├── classes │ └── pl │ └── samouczekprogramisty │ └── SomeServlet.class ├── lib │ └── some-jar-file.jar └── web.xml
...
Nadszedł czas, żeby wykorzystać tę wiedzę w praktyce. Proszę spójrz na przykład poniżej:
    ...
    @WebServlet("/date")public class DateServlet extends HttpServlet { @Override protected void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException { DateTime now = DateTime.now(); PrintWriter responseOutput = response.getWriter(); responseOutput.append("" + now.toString() + ""); }}
...
Taką klasę serwletu umieszczamy w projekcie. W moim przypadku jego struktura wygląda następująco:
    ...
    .├── 01_serwlety│ ├── build│ ├── build.gradle│ └── src│ └── main│ ├── java│ │ └── pl│ │ └── samouczekprogramisty│ │ └── kursaplikacjewebowe│ │ └── serwlety│ │ └── DateServlet.java│ └── webapp├── build.gradle├── gradle│ └── wrapper│ ├── gradle-wrapper.jar│ └── gradle-wrapper.properties├── gradlew├── gradlew.bat└── settings.gradle
...
[![dateservlet odpowiedz](http://www.samouczekprogramisty.pl/wp-content/uploads/2017/03/przegladarka_odpowiedz-300x181.jpeg)](http://www.samouczekprogramisty.pl/wp-content/uploads/2017/03/przegladarka_odpowiedz.jpeg)
